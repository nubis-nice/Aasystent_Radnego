import { createOpenAIClient, detectDocumentRelations, } from "@aasystent-radnego/shared";
export async function processRelations(job) {
    const { sourceDocumentId, sourceText, targetDocuments } = job.data;
    job.log(`Detecting relations for document ${sourceDocumentId} with ${targetDocuments.length} targets`);
    ...
    try {
        ...
        for (let i = 0; i < targetDocuments.length; i++) {
            ...
            if (!target)
                continue;
            job.log(`Checking relation with document ${target.id} (${i + 1}/${targetDocuments.length})`);
            const relation = await detectDocumentRelations(openai, sourceText, target.text);
