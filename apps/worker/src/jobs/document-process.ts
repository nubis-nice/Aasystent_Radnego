...
export async function processDocumentJob(
  ...
  try {
    ...
    const apiDistPath = path.resolve(
      __dirname,
    ...
    if (isAudioVideo) {
      ...
      const audioDistPath = path.resolve(
        __dirname,
        "../../../api/dist/services/audio-transcriber.js",
      );
      const { AudioTranscriber } = await import(
        pathToFileURL(audioDistPath).href
      );
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Nieznany błąd przetwarzania";
