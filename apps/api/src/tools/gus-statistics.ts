...
export const gusStatisticsTool: ToolDefinition = {
  name: "gus_statistics",
  description:
    "Pobiera statystyki demograficzne z GUS BDL (Bank Danych Lokalnych) - ludność, urodzenia, zgony, przyrost naturalny dla podanej gminy lub województwa",
  ...
  parameters: {
    ...
    properties: {
      unit_name: {
        type: "string",
        description:
          "Nazwa gminy lub województwa, np. 'Drawno', 'mazowieckie', 'Warszawa'",
      },
      data_type: {
        type: "string",
        description:
          "Typ danych do pobrania: population (ludność), births (urodzenia), deaths (zgony), all (wszystkie)",
        enum: ["population", "births", "deaths", "all"],
      },
      year: {
        type: "integer",
        description:
          "Rok dla którego pobrać dane (opcjonalnie, domyślnie najnowszy dostępny)",
      },
    },
    required: ["unit_name"],
  },
  ...
  execute: async (
    ...
    const unitName = args.unit_name as string;
    const dataType = (args.data_type as string) || "all";
    const year = args.year as number | undefined;

    console.log(
      `[GUS Tool] Executing: unit=${unitName}, type=${dataType}, year=${year || "latest"}`,
    );
    ...
    try {
      ...
      if (dataType !== "all") {
        const typeFilterIds: Record<string, string[]> = {
        ...
        if (filterIds.length > 0) {
          filteredVariables = stats.variables.filter((v) =>
            filterIds.includes(v.id),
          );
        }
      }
