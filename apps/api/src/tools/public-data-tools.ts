...
import type { ToolDefinition, ToolResult } from "../orchestrator/types.js";
...
// ISAP - Akty prawne
export const isapLegalTool: ToolDefinition = {
  name: "isap_legal",
  description:
    "Wyszukuje akty prawne w ISAP (Internetowy System Aktów Prawnych) - ustawy, rozporządzenia, dziennik ustaw",
...
export const geoportalTool: ToolDefinition = {
  name: "geoportal_spatial",
  description:
    "Wyszukuje dane przestrzenne z Geoportal.gov.pl - działki, adresy, MPZP, plany zagospodarowania",
  category: "spatial",
  parameters: {
    type: "object",
    properties: {
      ...
      coordinates: {
        type: "string",
        description:
          "Współrzędne geograficzne w formacie 'lat,lon' np. '52.23,21.01'",
      },
    },
    required: ["query"],
  },
  execute: async (args): Promise<ToolResult> => {
    const startTime = Date.now();
    const query = args.query as string;
    const coordinates = args.coordinates as string | undefined;
    ...
    try {
      ...
      if (results.addresses.length > 0 && results.parcels.length === 0) {
        ...
        if (firstAddress.coordinates?.lat && firstAddress.coordinates?.lon) {
          console.log(
            `[Geoportal] Using address coordinates to find parcel: ${firstAddress.coordinates.lat}, ${firstAddress.coordinates.lon}`,
          );
          const parcel = await geoportalService.getParcelByCoordinates(
            firstAddress.coordinates.lat,
            firstAddress.coordinates.lon,
          );
          if (parcel) {
      ...
      if (
        ...
        return {
          ...
          data: {
            ...
            query,
            warning:
              "Nie udało się znaleźć adresu w żadnym z serwisów geokodowania (GUGIK, OpenStreetMap).",
...
export const terytTool: ToolDefinition = {
  name: "teryt_registry",
  description:
    "Wyszukuje w rejestrze TERYT - gminy, powiaty, województwa, kody terytorialne, ulice",
  category: "public_data",
  parameters: {
    type: "object",
    properties: {
...
export const krsTool: ToolDefinition = {
  name: "krs_registry",
  description:
    "Wyszukuje w KRS - spółki, stowarzyszenia, fundacje. Można szukać po nazwie, NIP lub numerze KRS",
  category: "public_data",
  parameters: {
    type: "object",
    properties: {
...
export const ceidgTool: ToolDefinition = {
  name: "ceidg_registry",
  description:
    "Wyszukuje w CEIDG - jednoosobowe działalności gospodarcze. Można szukać po nazwie lub NIP",
  category: "public_data",
  parameters: {
    type: "object",
    properties: {
...
export const gdosTool: ToolDefinition = {
  name: "gdos_environmental",
  description:
    "Wyszukuje dane środowiskowe z GDOŚ - obszary Natura 2000, rezerwaty, parki narodowe, obszary chronione",
  category: "public_data",
  parameters: {
    type: "object",
    properties: {
...
export const euFundsTool: ToolDefinition = {
  name: "eu_funds",
  description:
    "Wyszukuje informacje o funduszach europejskich - dotacje, nabory, konkursy, projekty UE",
  category: "public_data",
  parameters: {
    type: "object",
    properties: {
  ...
  execute: async (args): Promise<ToolResult> => {
    const startTime = Date.now();
    const projectType = args.project_type as string | undefined;
    const municipality = args.municipality as string | undefined;
