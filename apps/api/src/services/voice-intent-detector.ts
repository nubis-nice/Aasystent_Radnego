...
export class VoiceIntentDetector {
  ...
  private llmModelName: string = "gpt-4o-mini";
  ...
  private async initialize(): Promise<void> {
    ...
    try {
      ...
      console.warn(
        "[VoiceIntent] Failed to get LLM config, using default model"
      );
    }
  ...
  private async loadAssistantName(): Promise<void> {
    try {
  ...
  private stripWakeWord(transcription: string): string {
    if (!this.assistantName) return transcription;

    const wakePatterns = [
      new RegExp(
        `^(hej|hey|cześć|witaj)?\\s*${this.assistantName}[,.]?\\s*`,
        "i"
      ),
  ...
  async detectIntent(transcription: string): Promise<DetectedIntent> {
    ...
    console.log(
      `[VoiceIntent] Original: "${transcription}" → Cleaned: "${cleanedTranscription}"`
    );
    ...
    try {
      ...
      console.log(
        `[VoiceIntent] Detected: ${result.intent} (${result.confidence})`
      );
  ...
  private detectQuickPatterns(transcription: string): DetectedIntent | null {
    const patterns = [
      {
        regex: /^(zatrzymaj|stop|pauza)$/i,
        intent: "control" as VoiceIntent,
  ...
  mapIntentToPath(intent: DetectedIntent): string | null {
    ...
    const target = intent.entities.target as string | undefined;
    if (target && pathMappings[target.toLowerCase()]) {
      return pathMappings[target.toLowerCase()];
    }

    const path = intent.entities.path as string | undefined;
    if (path) {
      return path;
    }
