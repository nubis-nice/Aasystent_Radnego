...
export class ExaProvider extends BaseResearchProvider {
  ...
  private async getContentsWithHighlights(
    ...
    try {
      ...
      const contentsResponse = await this.makeRequest<{
        results: ExaSearchResult[];
      }>("/contents", {
        method: "POST",
        body: JSON.stringify({
  ...
  protected transformResults(data: unknown): ResearchResult[] {
    const results = data as ExaSearchResult[];
  ...
  private createExcerpt(result: ExaSearchResult): string {
