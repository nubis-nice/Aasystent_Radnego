...
export class YouTubeDownloader {
  ...
  private async transcribeSingleFile(audioPath: string): Promise<string> {
    ...
    const contextPrompt =
      "Transkrypcja oficjalnego posiedzenia samorządowego w języku polskim. " +
  ...
  async transcribeAndAnalyze(
    ...
    videoTitle: string,
    videoUrl: string,
    precomputedParts?: AudioPart[], // Części z downloadAudio() - unikamy podwójnego split
    ...
    try {
      ...
      if (useChunkedTranscription) {
        ...
        if (precomputedParts && precomputedParts.length > 0) {
          console.log(
            `[YouTubeDownloader] Using ${precomputedParts.length} precomputed parts (no re-split)`,
          );
          ...
          if (splitResult.success && splitResult.parts.length > 0) {
            parts = splitResult.parts;
            console.log(
              `[YouTubeDownloader] Split into ${parts.length} chunks`,
            );
          } else {
            parts = [];
          }
        ...
        if (parts.length > 0) {
          console.log(
            `[YouTubeDownloader] Transcribing ${parts.length} chunks...`,
          );
  ...
  private removeRepetitions(text: string): string {
    ...
    console.log(
      `[YouTubeDownloader] removeRepetitions() input: ${originalLength} chars`,
    );
    ...
    if (elements.length > 10) {
      ...
      if (dedupedElements.length < elements.length * 0.5) {
        ...
        console.log(
          `[YouTubeDownloader] Comma dedup: ${elements.length} -> ${dedupedElements.length} elements`,
        );
      }
    }
    ...
    if (words.length > 20) {
      ...
      while (i < words.length) {
        ...
        for (let seqLen = 4; seqLen >= 1; seqLen--) {
          if (i + seqLen * 3 > words.length) continue;
      ...
      if (dedupedWords.length < words.length * 0.8) {
        ...
        console.log(
          `[YouTubeDownloader] Word seq dedup: ${words.length} -> ${dedupedWords.length} words`,
        );
      }
    }
    ...
    console.log(
      `[YouTubeDownloader] removeRepetitions() removed ${removedChars} chars (${removedPercent.toFixed(1)}%)`,
    );
    ...
    if (removedPercent > 85 && cleaned.length < 200) {
      console.log(
        `[YouTubeDownloader] Text is mostly repetitions (${removedPercent.toFixed(1)}%), audio has too much noise`,
      );
    ...
    if (
      ...
      console.log(
        `[YouTubeDownloader] Detected hallucination: only ${uniqueWordsCount} unique words from ${originalWordsCount} original words`,
      );
      return "[Za duże szumy w nagraniu - nie udało się rozpoznać mowy]";
    }
    ...
    if (maxPhraseCount > phrases.length * 0.5 && phrases.length > 5) {
      console.log(
        `[YouTubeDownloader] Detected repetitive hallucination: one phrase appears ${maxPhraseCount}/${phrases.length} times`,
      );
      return "[Za duże szumy w nagraniu - nie udało się rozpoznać mowy]";
    }
  ...
  private async correctTranscript(rawTranscript: string): Promise<string> {
    ...
    console.log(
      `[YouTubeDownloader] After dedup: ${deduped.length} chars (was ${rawTranscript.length})`,
    );
