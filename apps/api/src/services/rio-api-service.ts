...
export interface RIODecision {
  id: string;
  rio: string; // Nazwa RIO (np. "RIO w Poznaniu")
  rioCode: string; // Kod RIO (np. "poznan")
  decisionNumber: string; // Numer uchwały/rozstrzygnięcia
  decisionDate: string; // Data wydania (YYYY-MM-DD)
  decisionType: "uchwala" | "rozstrzygniecie" | "opinia" | "stanowisko";
  subject: string; // Przedmiot/temat
  municipality?: string; // Gmina której dotyczy
  legalBasis?: string[]; // Podstawa prawna
  summary?: string; // Streszczenie
  pdfUrl?: string; // Link do PDF
  sourceUrl: string; // URL źródła
}

export interface RIOSearchParams {
  rio?: string; // Kod RIO (np. "poznan", "warszawa")
  decisionType?: "uchwala" | "rozstrzygniecie" | "opinia" | "stanowisko";
  municipality?: string; // Nazwa gminy
...
export const RIO_CHAMBERS: Record<
  string,
  { name: string; url: string; province: string }
> = {
  bialystok: {
...
export const RIO_DECISION_TYPES: Record<string, string> = {
...
export class RIOApiService {
   ...
   */
  async searchDecisions(params: RIOSearchParams): Promise<RIOSearchResult> {
    const cacheKey = `search:${JSON.stringify(params)}`;

    if (this.cacheEnabled) {
      const cached = this.cache.get(cacheKey);
      if (cached && Date.now() - cached.timestamp < this.cacheTTL) {
        return cached.data as RIOSearchResult;
      }
    }
   ...
   */
  async getDecisionDetails(id: string): Promise<RIODecision | null> {
    const cacheKey = `decision:${id}`;

    if (this.cacheEnabled) {
      const cached = this.cache.get(cacheKey);
      if (cached && Date.now() - cached.timestamp < this.cacheTTL) {
        return cached.data as RIODecision;
      }
    }
   ...
   */
  getDecisionTypes(): Array<{ code: string; name: string }> {
  ...
  private async searchInChamber(
    ...
    try {
      ...
      const searchPath =
        params.decisionType === "rozstrzygniecie"
      ...
      if (!response.ok) {
        console.warn(
          `[RIO API] Failed to fetch from ${rioCode}: ${response.status}`,
        );
        return [];
      }
  ...
  private parseDecisionList(
    ...
    const items: RIODecision[] = [];
    const chamber = RIO_CHAMBERS[rioCode];
    if (!chamber) return items;
  ...
  private parseDecisionDetails(
    ...
    const dateMatch = html.match(
      /Data[:\s]+(\d{4}-\d{2}-\d{2}|\d{2}\.\d{2}\.\d{4})/i,
    );
  ...
  private extractDecisionNumber(title: string): string {
    const match = title.match(
      /(?:Nr|Numer|Uchwała|Rozstrzygnięcie)[:\s]*([^\s,]+)/i,
    );
    return match ? match[1] : "";
  }

  private detectDecisionType(text: string): RIODecision["decisionType"] {
    const lower = text.toLowerCase();
    if (lower.includes("rozstrzygnięci") || lower.includes("nadzorcz"))
      return "rozstrzygniecie";
    if (lower.includes("opini")) return "opinia";
