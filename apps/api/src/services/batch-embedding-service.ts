...
const DEFAULT_OPTIONS: Required<BatchEmbeddingOptions> = {
...
export class BatchEmbeddingService {
  ...
  private options: Required<BatchEmbeddingOptions>;
   ...
   */
  async createBatchJob(requests: BatchEmbeddingRequest[]): Promise<string> {
    ...
    if (requests.length > this.options.maxRequestsPerBatch) {
      throw new Error(
        `Too many requests: ${requests.length} > ${this.options.maxRequestsPerBatch}`
      );
    }

    console.log(
      `[BatchEmbedding] Creating batch job for ${requests.length} requests`
    );
    ...
    try {
      ...
      console.log(
        `[BatchEmbedding] Batch created: ${batch.id}, status: ${batch.status}`
      );
   ...
   */
  async getBatchResults(batchId: string): Promise<BatchEmbeddingResult[]> {
    ...
    console.log(
      `[BatchEmbedding] Downloading results from: ${status.outputFileId}`
    );
   ...
   */
  async listBatches(limit: number = 10): Promise<BatchJobStatus[]> {
  ...
  async waitForCompletion(
    ...
    while (true) {
      ...
      console.log(
        `[BatchEmbedding] Status: ${status.status}, ` +
  ...
  async processDocuments(
    ...
    const documentChunks: Map<string, string[]> = new Map();
    ...
    for (const doc of documents) {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        if (docChunks) docChunks.push(chunkId);
    ...
    console.log(
      `[BatchEmbedding] Processing ${documents.length} documents ` +
    ...
    console.log(
      `[BatchEmbedding] Generated embeddings for ${documentEmbeddings.size} documents`
    );
...
): { syncCost: number; batchCost: number; savings: number } {
