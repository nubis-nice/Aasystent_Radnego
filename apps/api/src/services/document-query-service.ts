...
export class DocumentQueryService {
  ...
  detectDocumentReferences(message: string): DocumentReference[] {
    ...
    // Uchwała nr X/Y/Z
    const uchwalPattern =
      /uchwał[aęy]\s*(?:nr|numer)?\s*([IVXLCDM]+\/\d+\/\d+|\d+\/\d+\/\d+)/gi;
    ...
    // Protokół z sesji X
    const protokolPattern =
      /protokoł?u?\s*(?:z\s*)?(?:sesji\s*)?(?:nr\s*)?([IVXLCDM]+|\d+)/gi;
    ...
    // Sesja nr X / X sesja
    const sesjaPattern =
      /(?:sesj[aię]\s*(?:nr\s*)?|(\d+)\s*sesj[aię])([IVXLCDM]+|\d+)?/gi;
   ...
   */
  async findDocumentById(documentId: string): Promise<DocumentMatch | null> {
   ...
   */
  async findDocumentsByTitle(title: string): Promise<DocumentMatch[]> {
    const { data, error } = await supabase
      .from("processed_documents")
      .select("id, title, document_type, publish_date, summary, source_url")
      .eq("user_id", this.userId)
   ...
   */
  async queryDocuments(message: string): Promise<DocumentQueryResult> {
   ...
   */
  private async getAttachments(documentId: string): Promise<DocumentMatch[]> {
    try {
      const { data, error } = await supabase
