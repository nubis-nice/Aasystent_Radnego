...
export interface EPUAPMessage {
  id: string;
  messageId: string; // ID wiadomości w ePUAP
  sender: string; // Nadawca (REGON/NIP)
  senderName: string; // Nazwa nadawcy
  recipient: string; // Odbiorca (adres ESP)
  subject: string; // Temat
  content?: string; // Treść (jeśli pobrana)
  attachments: EPUAPAttachment[];
  receivedAt: string; // Data odebrania
  status: "new" | "read" | "processed" | "archived";
  documentType?: string; // Typ dokumentu (wniosek, pismo, etc.)
  caseNumber?: string; // Numer sprawy
...
export interface EPUAPConfig {
  espAddress: string; // Adres ESP (Elektroniczna Skrzynka Podawcza)
  certificatePath?: string; // Ścieżka do certyfikatu
  certificatePassword?: string;
  wsdlUrl: string; // URL do WSDL
  testMode: boolean;
}
...
const DEFAULT_CONFIG: EPUAPConfig = {
  espAddress: "",
...
export class EPUAPService {
   ...
   */
  async getMessage(messageId: string): Promise<EPUAPMessage | null> {
    if (!this.userId) {
      throw new Error("Serwis nie został zainicjalizowany");
    }
   ...
   */
  async markAsProcessed(messageId: string, caseNumber?: string): Promise<void> {
    if (!this.userId) {
      throw new Error("Serwis nie został zainicjalizowany");
    }
   ...
   */
  async syncMessages(): Promise<{ synced: number; errors: string[] }> {
   ...
   */
  async handleWebhook(payload: EPUAPWebhookPayload): Promise<void> {
    console.log(
      "[ePUAP] Webhook received:",
      payload.eventType,
      payload.messageId,
    );
   ...
   */
  async saveConfig(config: Partial<EPUAPConfig>): Promise<void> {
    if (!this.userId) {
      throw new Error("Serwis nie został zainicjalizowany");
    }
  ...
  // Pomocnicze metody
  private mapDBToMessage(row: Record<string, unknown>): EPUAPMessage {
    return {
      id: row.id as string,
      messageId: row.epuap_message_id as string,
      sender: row.sender as string,
      senderName: row.sender_name as string,
      recipient: row.recipient as string,
      subject: row.subject as string,
      content: row.content as string | undefined,
      attachments: (row.attachments as EPUAPAttachment[]) || [],
      receivedAt: row.received_at as string,
      status: row.status as EPUAPMessage["status"],
