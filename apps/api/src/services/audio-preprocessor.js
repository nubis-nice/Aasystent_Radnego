...
import { getAudioAnalyzer, } from "./audio-analyzer.js";
...
export class AudioPreprocessor {
     ...
     */
    async preprocess(inputBuffer, inputFileName, options = {}) {
        const opts = { ...DEFAULT_OPTIONS, ...options };
        // Zapisz input do pliku tymczasowego
        const inputPath = path.join(this.tempDir, `input-${Date.now()}-${inputFileName}`);
        const outputPath = path.join(this.tempDir, `output-${Date.now()}.${opts.outputFormat}`);
        fs.writeFileSync(inputPath, inputBuffer);
        try {
            // Zbuduj łańcuch filtrów audio
            const filterChain = this.buildFilterChain(opts);
            console.log(`[AudioPreprocessor] Processing: ${inputFileName} with filters: ${filterChain}`);
            // Uruchom ffmpeg
            ...
            console.log(`[AudioPreprocessor] Processed: ${inputFileName} -> ${outputPath}`);
            console.log(`[AudioPreprocessor] Size: ${(inputBuffer.length / 1024 / 1024).toFixed(2)}MB -> ${(outputBuffer.length / 1024 / 1024).toFixed(2)}MB`);
     ...
     */
    async preprocessAdaptive(inputPath, outputFormat = "wav") {
        if (!fs.existsSync(inputPath)) {
            throw new Error(`Plik nie istnieje: ${inputPath}`);
        }
        console.log(`[AudioPreprocessor] Starting adaptive preprocessing: ${inputPath}`);
        // 1. Analiza audio
        ...
        console.log(`[AudioPreprocessor] Issues detected: ${analysis.issues.map((i) => i.type).join(", ") || "none"}`);
        // 3. Przygotuj ścieżkę wyjściową
        const outputPath = path.join(this.tempDir, `adaptive-${Date.now()}.${outputFormat}`);
    ...
    buildAdaptiveFilterChain(recs) {
        ...
        if (recs.enableCompressor) {
            ...
            filters.push(`acompressor=threshold=${recs.compressorThreshold}dB:ratio=${recs.compressorRatio}:attack=5:release=50:makeup=2`);
        }
    ...
    buildFilterChain(opts) {
        ...
        if (opts.noiseReduction) {
            ...
            filters.push(`afftdn=nr=${opts.noiseReductionStrength * 100}:nf=-25:tn=1`);
        }
        ...
        if (opts.deesser) {
            ...
            filters.push("highpass=f=4000,compand=attacks=0:decays=0.3:points=-80/-80|-12/-12|-6/-9|0/-6:soft-knee=6,lowpass=f=8000");
        }
        // 6. Kompresor dynamiki - wyrównuje głośność (ciche fragmenty głośniejsze, głośne cichsze)
        if (opts.compressor) {
            // threshold:ratio:attack:release:makeup
            filters.push(`compand=attacks=0.1:decays=0.3:points=-70/-70|${opts.compressorThreshold}/${opts.compressorThreshold}|0/${-opts.compressorThreshold / opts.compressorRatio}:soft-knee=6:gain=3`);
        }
        ...
        if (opts.normalizeVolume) {
            ...
            filters.push(`loudnorm=I=${opts.targetLoudness}:TP=-1.5:LRA=11:print_format=summary`);
        }
        return filters.join(",");
    }
    /**
     ...
     */
    runFFmpeg(inputPath, outputPath, filterChain, opts) {
     ...
     */
    async splitAudioByTime(inputPath, maxPartDuration = 600) {
        try {
            ...
            console.log(`[AudioPreprocessor] Total duration: ${totalDuration.toFixed(1)}s`);
            if (totalDuration <= maxPartDuration) {
                console.log(`[AudioPreprocessor] Audio shorter than maxPartDuration, no splitting needed`);
                return {
                    success: true,
                    parts: [],
            ...
            while (currentTime < totalDuration) {
                ...
                console.log(`[AudioPreprocessor] Extracting part ${partIndex}: ${startTime.toFixed(1)}s - ${endTime.toFixed(1)}s`);
