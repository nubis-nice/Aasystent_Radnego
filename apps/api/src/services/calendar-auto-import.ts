...
interface ExtractedEventInfo {
  ...
  location?: string; // Miejsce wydarzenia
  reasoning?: string; // Uzasadnienie dlaczego to obowiązek radnego
  isCouncilDuty?: boolean; // Czy to obowiązek radnego
}
...
export async function autoImportToCalendar(doc: DocumentInfo): Promise<void> {
  ...
  if (duplicateCheck && duplicateCheck.length > 0) {
    for (const existing of duplicateCheck) {
      const titleSimilar =
        existing.title.toLowerCase() === eventTitle.toLowerCase() ||
        existing.title
          .toLowerCase()
      ...
      messages: [
        {
          role: "system",
          content:
            "Jesteś asystentem wyodrębniającym informacje o wydarzeniach z dokumentów urzędowych. Odpowiadaj tylko w formacie JSON.",
        },
 ...
 */
export async function batchImportExistingDocuments(userId: string): Promise<{
  imported: number;
  skipped: number;
  errors: number;
}> {
