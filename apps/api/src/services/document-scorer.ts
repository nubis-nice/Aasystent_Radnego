...
export class DocumentScorer {
  private councilLocation: string;

  constructor(councilLocation: string = "Drawno") {
  ...
  calculateScore(doc: {
    ...
    const typeScore =
      TYPE_WEIGHTS[doc.document_type] ?? TYPE_WEIGHTS.other ?? 30;
   ...
   */
  async updateDocumentScore(documentId: string): Promise<DocumentScore | null> {
