...
export async function gdosRoutes(fastify: FastifyInstance) {
  ...
  fastify.get(
    ...
    async (
      ...
      try {
        const { lat, lon, name, type, voivodeship, municipality, limit } =
          request.query;

        // Wyszukiwanie po lokalizacji
        if (lat && lon) {
          const areas = await gdosService.getProtectedAreasAtLocation(
            parseFloat(lat),
            parseFloat(lon)
          );
          return reply.send({
        ...
        const areas = await gdosService.searchProtectedAreas({
          ...
          type: type as Parameters<
            typeof gdosService.searchProtectedAreas
          >[0]["type"],
          voivodeship,
          municipality,
  ...
  fastify.get(
    ...
    async (
      ...
      try {
        ...
        // Wyszukiwanie po lokalizacji
        if (lat && lon) {
          const sites = await gdosService.getNatura2000AtLocation(
            parseFloat(lat),
            parseFloat(lon)
          );
          return reply.send({
        ...
        if (name) {
          const sites = await gdosService.searchNatura2000(
            name,
