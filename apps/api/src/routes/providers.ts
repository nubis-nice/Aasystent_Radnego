...
import { ProviderCapability, ProviderType } from "@aasystent-radnego/shared";
...
export async function providerRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/providers/capabilities", async (_request, reply) => {
    try {
      ...
      fastify.log.error(
        "Error fetching provider capabilities: " +
  ...
  }>("/providers/:provider/defaults", async (request, reply) => {
    try {
      ...
      fastify.log.error(
        "Error fetching provider defaults: " +
          String(error instanceof Error ? error.message : error),
      );
      return reply
        .status(500)
  ...
  fastify.get("/providers/supported", async (_request, reply) => {
    try {
      ...
      fastify.log.error(
        "Error fetching supported providers: " +
          String(error instanceof Error ? error.message : error),
      );
      return reply
        .status(500)
