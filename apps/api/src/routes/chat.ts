    ...
    try {
      ...
      if (!apiKey) {
        return reply.status(400).send({
          error:
            "Brak konfiguracji API. Przejdź do Ustawienia → Konfiguracja API i dodaj klucz API.",
        });
      }
      ...
      if (
        ...
        if (ragContext.municipalData.length > 0) {
          ...
          ragContext.municipalData.forEach((item, idx) => {
            contextMessage += `### [${idx + 1}] ${item.title} (${
              item.dataType
            })\n${item.content}\n\n`;
          });
        }
      ...
      if (
        ...
        return reply.status(402).send({
          ...
          message:
            "Wyczerpano limit API OpenAI. Doładuj konto lub zmień klucz API.",
          details:
            "Twój klucz API OpenAI wyczerpał dostępny limit. Aby kontynuować korzystanie z Asystenta AI, musisz doładować konto OpenAI.",
        ...
        messages: [
          {
            role: "system",
            content:
              "Jesteś asystentem tworzącym zwięzłe i merytoryczne podsumowania dokumentów. Twoim zadaniem jest przeanalizować podane dokumenty i stworzyć kompleksowe podsumowanie.",
          },
          {
            role: "user",
