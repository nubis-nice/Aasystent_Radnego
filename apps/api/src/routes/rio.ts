...
export const rioRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  }>("/rio/decisions/search", async (request, reply) => {
    try {
      const {
        rio,
        decisionType,
  ...
  fastify.get<{
    Params: { id: string };
  }>("/rio/decisions/:id", async (request, reply) => {
    try {
      const { id } = request.params;
      const decision = await rioService.getDecisionDetails(
  ...
  }>("/rio/decisions/municipality", async (request, reply) => {
    try {
      ...
      const decisions = await rioService.searchByMunicipality(
        municipality,
      ...
      fastify.log.error(
        "Error fetching municipality decisions: " +
          String(error instanceof Error ? error.message : error),
      );
      return reply.status(500).send({
        success: false,
  ...
  }>("/rio/decisions/budget", async (request, reply) => {
    try {
      const { rio, limit } = request.query;

      const decisions = await rioService.searchBudgetDecisions(
        rio,
        limit ? parseInt(limit) : 20,
      );
      ...
      fastify.log.error(
        "Error fetching budget decisions: " +
          String(error instanceof Error ? error.message : error),
      );
      return reply.status(500).send({
        success: false,
