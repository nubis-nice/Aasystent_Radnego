...
export async function legalAnalysisRoutes(fastify: FastifyInstance) {
  ...
  }>("/legal/search", async (request, reply) => {
    try {
  ...
  }>("/legal/reasoning", async (request, reply) => {
    try {
      const userId = request.headers["x-user-id"] as string;
      if (!userId) {
        return reply.status(401).send({ error: "Unauthorized" });
  ...
  }>("/legal/budget-analysis", async (request, reply) => {
    try {
      const userId = request.headers["x-user-id"] as string;
      if (!userId) {
        return reply.status(401).send({ error: "Unauthorized" });
