...
export class UniversalToolOrchestrator {
  ...
  async process(
    ...
    console.log(
      `[Orchestrator] Processing: "${userMessage.substring(0, 50)}..."`,
    );
    ...
    let response: string;
    let uiAction: OrchestratorResult["uiAction"];
   ...
   */
  private async processWithNativeTools(
    userMessage: string,
    context: ToolContext,
  ): Promise<{
    response: string;
    toolsUsed: string[];
    uiAction?: OrchestratorResult["uiAction"];
    reasoning?: string;
  }> {
    const toolsUsed: string[] = [];
    ...
    try {
        ...
        messages: [
          {
            role: "system",
            content:
              "Jesteś asystentem radnego. Używaj dostępnych narzędzi gdy potrzebne.",
          },
      ...
      for (const toolCall of message.tool_calls) {
        ...
        const callData = toolCall as {
          type: string;
          function?: { name: string; arguments: string };
          id: string;
        };
        if (callData.type !== "function" || !callData.function) continue;
   ...
   */
  private async processWithPromptBased(
    userMessage: string,
    context: ToolContext,
  ): Promise<{
    response: string;
    ...
    const systemPrompt = TOOL_SELECTION_SYSTEM_PROMPT.replace(
      "{TOOLS}",
      toolsDescription,
    );
    ...
    try {
      ...
      // Parse JSON response
      let selection: PromptToolSelection;
      try {
        ...
        console.error(
          "[Orchestrator] Failed to parse tool selection:",
          content,
        );
        return { response: await this.simpleChat(userMessage), toolsUsed: [] };
      }
      ...
      console.log(
        `[Orchestrator] Tool selection: ${selection.action}, thought: ${selection.thought}`,
      );
      ...
      if (!tool) {
        console.warn(
          `[Orchestrator] Unknown tool selected: ${selection.action}`,
        );
        return { response: await this.simpleChat(userMessage), toolsUsed: [] };
      }
      ...
      try {
        ...
        // Syntezuj odpowiedź
        const synthesis = await this.synthesizeFromData(
          userMessage,
          selection.action,
          result,
        );
   ...
   */
  private async synthesizeFromData(
    userMessage: string,
    toolName: string,
    result: ToolResult,
  ): Promise<string> {
   ...
   */
  private async simpleChat(userMessage: string): Promise<string> {
    const response = await this.client.chat.completions.create({
      model: this.config.model,
      messages: [
   ...
   */
  private async checkNativeToolSupport(): Promise<boolean> {
    // OpenAI modele
    ...
    if (this.config.provider === "ollama" || this.config.provider === "local") {
      ...
      if (hasSupport) {
        ...
        try {
          ...
          console.log(
            `[Orchestrator] Model ${this.config.model} supports native tools`,
          );
          return true;
        } catch {
          console.log(
            `[Orchestrator] Model ${this.config.model} does not support native tools`,
          );
          return false;
        }
      }
