...
DROP POLICY IF EXISTS "Users can manage own appearance settings" ON user_appearance_settings;
...
CREATE INDEX IF NOT EXISTS idx_locale_settings_user ON user_locale_settings(user_id);

ALTER TABLE user_locale_settings ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can manage own locale settings" ON user_locale_settings;
...
DROP TRIGGER IF EXISTS update_locale_settings_updated_at ON user_locale_settings;
...
-- 11. Inicjalizacja ustawień dla istniejących użytkowników
DO $$
DECLARE
  user_record RECORD;
BEGIN
  FOR user_record IN SELECT id, raw_user_meta_data FROM auth.users LOOP
    ...
    VALUES (user_record.id, COALESCE(user_record.raw_user_meta_data->>'full_name', 'Użytkownik'))
    ON CONFLICT (id) DO NOTHING;
