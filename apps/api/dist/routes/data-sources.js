...
import { ScrapingQueueManager, calculateSourcePriority, } from "../services/scraping-queue.js";
...
export async function dataSourcesRoutes(fastify) {
    ...
    fastify.post("/data-sources/:id/scrape", async (request, reply) => {
        try {
            ...
            const { id } = request.params;
            const { maxDocumentAgeDays } = request.body || {};
            // Sprawdź czy źródło należy do użytkownika
            ...
            if (maxDocumentAgeDays && maxDocumentAgeDays > 0) {
                ...
                request.log.info({ maxDocumentAgeDays, minDocumentDate }, "Filtering documents by age");
            }
            ...
            request.log.info({ sourceId: id, jobId, priority, maxDocumentAgeDays }, "Scraping job enqueued");
    ...
    fastify.post("/data-sources/scrape-all", async (request, reply) => {
        try {
            const userId = request.headers["x-user-id"];
            if (!userId) {
                return reply.status(401).send({ error: "Unauthorized" });
            ...
            if (excludeApiSources) {
                sourcesToScrape = sourcesToScrape.filter((s) => !API_SOURCE_TYPES.includes(s.type));
            }
            // Filtruj tylko aktywne źródła
            sourcesToScrape = sourcesToScrape.filter((s) => s.scraping_enabled !== false);
            ...
            // Dodaj do kolejki
            const queueManager = ScrapingQueueManager.getInstance();
            const result = await queueManager.enqueueBulk(sourcesToScrape.map((s) => ({
                sourceId: s.id,
                userId,
                sourceType: s.type,
            })));
            request.log.info({ queued: result.totalQueued, skipped: result.skipped.length }, "Bulk scraping enqueued");
            return reply.send({
    ...
    fastify.get("/data-sources/queue/job/:jobId", async (request, reply) => {
        try {
            const userId = request.headers["x-user-id"];
            if (!userId) {
                return reply.status(401).send({ error: "Unauthorized" });
