...
export async function voiceRoutes(fastify) {
    fastify.post("/voice/transcribe", async (request, reply) => {
        ...
        try {
            ...
            console.log(`[Voice] STT completed: ${transcription.text.substring(0, 50)}... (${duration}ms)`);
    ...
    fastify.post("/voice/synthesize", async (request, reply) => {
        ...
        console.log(`[Voice TTS] Original: ${text.length} chars → Processed: ${processedText.length} chars`);
        try {
     ...
     */
    fastify.post("/voice/detect-wake-word", async (request, reply) => {
