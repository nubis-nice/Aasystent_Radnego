/* global URL */
import { semanticWebSearch, } from "../services/semantic-web-search.js";
export async function webSearchRoutes(fastify) {
    ...
    fastify.post("/web-search", async (request, reply) => {
        try {
            ...
            const { query, maxResults, minCredibility, requireCrossReference, excludeDomains, preferredDomains, } = request.body;
    ...
    fastify.post("/web-search/verify-url", async (request, reply) => {
        try {
            const userId = request.headers["x-user-id"];
            if (!userId) {
                return reply.status(401).send({ error: "Unauthorized" });
