...
export async function krsRoutes(fastify) {
    const krsService = new KrsService();
    // GET /api/krs/entity/:krsNumber - podmiot po numerze KRS
    fastify.get("/krs/entity/:krsNumber", async (request, reply) => {
        try {
    ...
    fastify.get("/krs/nip/:nip", async (request, reply) => {
        try {
    ...
    fastify.get("/krs/regon/:regon", async (request, reply) => {
        try {
    ...
    fastify.get("/krs/entity/:krsNumber/representatives", async (request, reply) => {
        try {
            const { krsNumber } = request.params;
    ...
    fastify.get("/krs/entity/:krsNumber/pkd", async (request, reply) => {
        try {
            const { krsNumber } = request.params;
    ...
    fastify.get("/krs/search", async (request, reply) => {
        try {
            const { name, krs, nip, regon, page, limit } = request.query;
            if (!name && !krs && !nip && !regon) {
                return reply.status(400).send({
                    success: false,
                    error: "Wymagany co najmniej jeden parametr wyszukiwania: name, krs, nip lub regon",
                });
            }
