...
export async function testRoutes(fastify) {
    ...
    fastify.post("/test/connection", async (request, reply) => {
        try {
            ...
            const { config_id, provider, api_key, base_url, auth_method } = request.body;
            let config;
            if (config_id) {
                ...
                if (dbConfig.encryption_iv && dbConfig.encryption_iv.length > 0) {
                    ...
                    decryptedKey = decryptApiKey(dbConfig.api_key_encrypted, dbConfig.encryption_iv);
                }
                else {
                    // Stary format - base64
                    decryptedKey = Buffer.from(dbConfig.api_key_encrypted, "base64").toString("utf-8");
                }
            ...
            if (config_id) {
                ...
                await supabase
                    .from("api_configurations")
                    .update({
                    connection_status: result.status === "success" ? "working" : "failed",
    ...
    fastify.post("/test/chat", async (request, reply) => {
        try {
            const userId = request.headers["x-user-id"];
            if (!userId) {
                return reply.status(401).send({ error: "Unauthorized" });
            }
            const { config_id, message = "Hello, this is a test message." } = request.body;
            const { data: dbConfig, error } = await supabase
                .from("api_configurations")
                .select("*")
                .eq("id", config_id)
            ...
            const decryptedKey = decryptApiKey(dbConfig.api_key_encrypted, dbConfig.encryption_iv || "");
            const config = {
                provider: dbConfig.provider,
                apiKey: decryptedKey,
    ...
    fastify.post("/test/embeddings", async (request, reply) => {
        try {
            const userId = request.headers["x-user-id"];
            if (!userId) {
                return reply.status(401).send({ error: "Unauthorized" });
    ...
    fastify.post("/test/models", async (request, reply) => {
        try {
            const userId = request.headers["x-user-id"];
            if (!userId) {
                return reply.status(401).send({ error: "Unauthorized" });
