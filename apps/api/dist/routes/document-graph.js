...
export const documentGraphRoutes = async (fastify) => {
    // GET /documents/:id/related - Pobierz powiązane dokumenty
    fastify.get("/documents/:id/related", async (request, reply) => {
        ...
        try {
            ...
            const related = await documentGraphService.getRelatedDocuments(id, query.maxDepth, query.minStrength);
            return reply.send({
                document_id: id,
    ...
    fastify.get("/documents/:id/relations", async (request, reply) => {
        const { id } = request.params;
        const userId = request.headers["x-user-id"];
        if (!userId) {
            return reply.status(401).send({ error: "Unauthorized" });
    ...
    fastify.get("/documents/path", async (request, reply) => {
        ...
        try {
            const query = querySchema.parse(request.query);
            const path = await documentGraphService.findPath(query.source, query.target, query.maxDepth);
            if (!path) {
                return reply.send({
                    found: false,
    ...
    fastify.post("/documents/:id/detect-references", async (request, reply) => {
        const { id } = request.params;
        const userId = request.headers["x-user-id"];
        if (!userId) {
            return reply.status(401).send({ error: "Unauthorized" });
    ...
    fastify.post("/documents/:id/relations", async (request, reply) => {
        const { id } = request.params;
        const userId = request.headers["x-user-id"];
        if (!userId) {
            return reply.status(401).send({ error: "Unauthorized" });
        ...
        try {
            ...
            const relation = await documentGraphService.addRelation(id, body.target_document_id, body.relation_type, {
    ...
    fastify.delete("/documents/relations/:id", async (request, reply) => {
        const { id } = request.params;
        const userId = request.headers["x-user-id"];
        if (!userId) {
            return reply.status(401).send({ error: "Unauthorized" });
    ...
    fastify.post("/documents/clusters", async (request, reply) => {
        ...
        try {
            const body = bodySchema.parse(request.body);
            const cluster = await documentGraphService.createCluster(body.name, body.root_document_id, {
...
//# sourceMappingURL=document-graph.js.map