...
export class BraveProvider extends BaseResearchProvider {
    async search(query, options = {}) {
        ...
        try {
            ...
            if (request.safesearch)
                params.append("safesearch", request.safesearch);
            if (request.freshness)
                params.append("freshness", request.freshness);
            if (request.extra_snippets)
                params.append("extra_snippets", "true");
            const response = await this.makeRequest(`/res/v1/web/search?${params.toString()}`, { method: "GET" });
    ...
    transformResults(data) {
        ...
        return results.map((result, index) => {
            ...
            const positionBasedScore = 0.95 - (index / Math.max(totalResults, 1)) * 0.45;
            return {
                id,
    ...
    detectDocumentType(url) {
        ...
        if (domain.includes("bip"))
            return "dokument_bip";
        if (domain.includes("sejm"))
            return "dokument_sejm";
        if (url.includes(".pdf"))
            return "pdf";
        return undefined;
    }
...
//# sourceMappingURL=brave-provider.js.map