...
export class SemanticDocumentDiscovery {
    ...
    async search(query) {
        ...
        try {
            ...
            console.log(`[SemanticDiscovery] Found ${ragResults.length} RAG documents`);
            // PHASE 2: Search w scraped_content (surowe dane)
            const scrapedResults = await this.searchScrapedContent(query);
            console.log(`[SemanticDiscovery] Found ${scrapedResults.length} scraped documents`);
            ...
            const scoredResults = await this.scoreRelevance(mergedResults, query.query);
            ...
            const shouldRunIntelligentScraping = query.enableIntelligentScraping &&
                filteredResults.length < minResultsBeforeScraping &&
                query.sourceId;
            if (shouldRunIntelligentScraping) {
                console.log(`[SemanticDiscovery] Not enough results (${filteredResults.length}), running intelligent scraping...`);
                const scrapingResults = await this.runIntelligentScraping(query);
                if (scrapingResults.newDocuments > 0) {
                    console.log(`[SemanticDiscovery] Intelligent scraping found ${scrapingResults.newDocuments} new documents, re-searching...`);
            ...
            result.errors.push(`Search error: ${error instanceof Error ? error.message : "Unknown"}`);
        }
        result.processingTimeMs = Date.now() - startTime;
        console.log(`[SemanticDiscovery] Completed in ${result.processingTimeMs}ms, found ${result.totalFound} documents`);
        return result;
    }
    // ============================================================================
    ...
    async searchRAGDocuments(query) {
        ...
        try {
            ...
            const { data: documents, error } = await supabase.rpc("match_documents_semantic", {
            ...
            if (error) {
                ...
                console.log("[SemanticDiscovery] RPC not available, using fallback search");
                return this.fallbackRAGSearch(query);
            }
            return (documents || []).map((doc) => this.transformRAGDocument(doc));
        }
        catch (error) {
    ...
    // ============================================================================
    async searchScrapedContent(query) {
        ...
        dbQuery = dbQuery.textSearch("raw_content", query.query.split(" ").join(" | "));
        const { data: documents, error } = await dbQuery.limit(query.maxResults || 30);
        if (error) {
            console.error("[SemanticDiscovery] Scraped content search error:", error);
            return [];
        }
        return (documents || []).map((doc) => this.transformScrapedDocument(doc));
    }
    transformScrapedDocument(doc) {
        return {
            id: doc.id,
            url: doc.url || "",
            title: doc.title || "Bez tytułu",
                        ...
                        content: `Oceń trafność dokumentów dla zapytania. Odpowiedz w JSON:
{
  "scores": [
                ...
                for (const score of result.scores) {
                    ...
                    if (doc) {
                        ...
                        doc.llmAnalysis = {
                            ...
                            recommendedAction: score.relevance > 0.7
                                ? "priority"
                                : score.relevance > 0.3
                                    ? "include"
                                    : "skip",
    ...
    // PHASE 5: DEEP CRAWL
    // ============================================================================
    async deepCrawlDocuments(documents, query) {
        let newDocsCount = 0;
        ...
        for (const doc of topDocs) {
            ...
            try {
                ...
                const relevantLinks = await this.filterRelevantLinks(links, query.query);
    ...
    async filterRelevantLinks(links, query) {
        // Proste filtrowanie po słowach kluczowych
        const keywords = query.toLowerCase().split(/\s+/);
        return links
            .filter((link) => {
            const linkLower = link.toLowerCase();
            return (keywords.some((kw) => linkLower.includes(kw)) ||
    ...
    async crawlAndSave(url, sourceId) {
        try {
    ...
    // ============================================================================
    async extractPDFDocuments(documents) {
    ...
    async processPDF(url, sourceId) {
        try {
            ...
            const result = await processor.processFile(Buffer.from(buffer), url.split("/").pop() || "document.pdf", "application/pdf");
    ...
    // ============================================================================
    async runIntelligentScraping(query) {
        const result = { newDocuments: 0, errors: [] };
        if (!query.sourceId) {
            return result;
        }
        try {
            ...
            console.log(`[SemanticDiscovery] Running intelligent scraping on: ${source.url}`);
            // Uruchom IntelligentScraper z konfiguracją opartą na zapytaniu
            const scraper = new IntelligentScraper(source.url, this.userId, query.sourceId, {
            ...
            console.log(`[SemanticDiscovery] Intelligent scraping completed: ${result.newDocuments} new documents`);
        }
        catch (error) {
            result.errors.push(`Intelligent scraping error: ${error instanceof Error ? error.message : "Unknown"}`);
        }
        return result;
    }
...
// EXPORTED FUNCTION
// ============================================================================
export async function semanticDocumentSearch(userId, query) {
    const discovery = new SemanticDocumentDiscovery(userId);
    return discovery.search(query);
}
//# sourceMappingURL=semantic-document-discovery.js.map