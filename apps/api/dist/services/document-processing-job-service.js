...
export class DocumentProcessingJobService {
    ...
    async createJob(options) {
        ...
        this.processJob(jobId, options.fileBuffer, options.mimeType).catch((error) => {
     ...
     */
    async processJob(jobId, fileBuffer, mimeType) {
        const job = jobQueue.get(jobId);
        if (!job)
            return;
        try {
            ...
            const result = await processor.processFile(fileBuffer, job.fileName, mimeType);
            ...
            if (job.formatAsProfessional) {
                ...
                formattedContent = await this.formatDocument(content, this.isAudioFile(mimeType));
            }
            // 5. Zapis do RAG
            if (job.saveToRag) {
                this.updateJob(jobId, {
                ...
                const documentType = this.isAudioFile(mimeType)
                    ? "transkrypcja"
                    : "ocr";
                const saveResult = await processor.saveToRAG(this.userId, content, job.fileName, job.fileName, documentType);
            ...
            console.log(`[DocumentProcessingJob] Job ${jobId} completed successfully`);
        }
        catch (error) {
...
Format odpowiedzi (tylko JSON, bez markdown):
{
  ...
  "summary": "krótkie podsumowanie sentymentu"
     ...
     */
    async formatDocument(content, isTranscription) {
        try {
            const llmClient = await getLLMClient(this.userId);
            const config = await getAIConfig(this.userId, "llm");
            const response = await llmClient.chat.completions.create({
...
//# sourceMappingURL=document-processing-job-service.js.map