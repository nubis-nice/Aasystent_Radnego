...
export class UnifiedDataService {
    sourceId;
    userId;
    constructor(sourceId, userId) {
        this.sourceId = sourceId;
        this.userId = userId;
    }
    async fetchAndProcess() {
        const startTime = Date.now();
        console.log(`[UnifiedDataService] ========================================`);
        console.log(`[UnifiedDataService] Starting fetch for sourceId: ${this.sourceId}`);
        console.log(`[UnifiedDataService] User: ${this.userId}`);
        try {
            const config = await this.loadSourceConfig();
            if (!config) {
                console.error(`[UnifiedDataService] ERROR: Source configuration not found!`);
                return this.createErrorResult("Source configuration not found", startTime);
            }
            ...
            console.log(`[UnifiedDataService] Has scraperConfig: ${!!config.scraperConfig}`);
            ...
            if (documents.length === 0) {
                ...
                console.warn(`[UnifiedDataService] Check if apiConfig/scraperConfig is properly configured`);
            }
            console.log(`[UnifiedDataService] Saving documents...`);
            await this.saveDocuments(documents);
            console.log(`[UnifiedDataService] Processing documents (generating embeddings)...`);
            ...
            console.log(`[UnifiedDataService] Items fetched: ${documents.length}, processed: ${processedCount}`);
            console.log(`[UnifiedDataService] ========================================`);
            return {
    ...
    async processDocuments(documents) {
        ...
        const { data: apiConfig } = await supabase
            ...
            .single();
        if (!apiConfig) {
            console.warn("[UnifiedDataService] No OpenAI API config found, skipping processing");
            return 0;
        }
        ...
        const embeddingModel = apiConfig.embedding_model || "text-embedding-3-small";
        for (const doc of documents) {
            try {
                const { data: existing } = await supabase
    ...
    classifyDocument(title, content) {
        ...
        if (lowerTitle.includes("protokół") ||
            lowerContent.includes("protokół z")) {
            return "protocol";
        }
        if (lowerTitle.includes("ogłoszeni") ||
    ...
    async updateSourceStatus(success, errorMessage) {
        const updates = {
    ...
    async logFetchComplete(fetched, processed) {
        await supabase.from("scraping_logs").insert({
            source_id: this.sourceId,
            status: "success",
    ...
    async logFetchError(errorMessage) {
        await supabase.from("scraping_logs").insert({
            source_id: this.sourceId,
            status: "error",
     ...
     */
    async processPDFAttachments(documents) {
        let processedPdfCount = 0;
        // Extract all PDF links from scraped documents
        const pdfLinks = [];
        for (const doc of documents) {
            const docPdfLinks = doc.rawData?.pdfLinks || [];
        ...
        console.log(`[UnifiedDataService] Found ${pdfLinks.length} PDF links to process`);
        ...
        try {
            ...
            console.error("[UnifiedDataService] Failed to initialize DocumentProcessor:", error);
            return 0;
        }
        ...
        for (const pdfUrl of pdfsToProcess) {
            try {
                ...
                if (!response.ok) {
                    console.warn(`[UnifiedDataService] Failed to download PDF: ${response.status}`);
                    continue;
                }
                ...
                console.log(`[UnifiedDataService] Processing PDF: ${fileName} (${pdfBuffer.length} bytes)`);
                ...
                if (!result.success || !result.text || result.text.length < 100) {
                    console.warn(`[UnifiedDataService] PDF processing failed or text too short: ${fileName}`);
                    continue;
                }
                console.log(`[UnifiedDataService] Extracted ${result.text.length} chars from ${fileName}`);
                // Save to RAG
                const saveResult = await processor.saveToRAG(this.userId, result.text, fileName.replace(".pdf", ""), fileName, "pdf_attachment");
                if (saveResult.success) {
                    processedPdfCount++;
                    console.log(`[UnifiedDataService] Saved PDF to RAG: ${fileName} (ID: ${saveResult.documentId})`);
                }
            }
            catch (error) {
                console.error(`[UnifiedDataService] Error processing PDF ${pdfUrl}:`, error);
            }
        }
