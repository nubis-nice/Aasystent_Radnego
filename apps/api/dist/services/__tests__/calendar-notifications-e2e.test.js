...
function getUpcomingNotifications(userId, now) {
    const next24h = new Date(now.getTime() + 24 * 60 * 60 * 1000);
    // Filtruj wydarzenia użytkownika w zakresie 24h
    const userEvents = mockDatabase.events.filter((e) => e.user_id === userId &&
...
function deleteEvent(userId, eventId) {
    const index = mockDatabase.events.findIndex((e) => e.id === eventId && e.user_id === userId);
    if (index !== -1) {
...
async function runE2ETests() {
    ...
    await test("2.1 Utworzenie 3 wydarzeń", () => {
        ...
        createEvent("user-e2e-2", {
            ...
            start_date: new Date(baseTime.getTime() + 2 * 60 * 60 * 1000).toISOString(),
            reminder_minutes: [60],
        });
        ...
        createEvent("user-e2e-2", {
            ...
            event_type: "meeting",
            start_date: new Date(baseTime.getTime() + 5 * 60 * 60 * 1000).toISOString(),
            reminder_minutes: [60],
        });
    ...
    await test("100 wydarzeń - czas odpowiedzi < 100ms", () => {
        ...
        for (let i = 0; i < 100; i++) {
            createEvent("stress-user", {
                title: `Wydarzenie ${i}`,
                event_type: "meeting",
                start_date: new Date(baseTime.getTime() + (i + 1) * 60 * 60 * 1000).toISOString(),
                reminder_minutes: [60],
            });
        }
...
//# sourceMappingURL=calendar-notifications-e2e.test.js.map