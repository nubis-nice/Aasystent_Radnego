...
function runTests() {
    ...
    console.log(`   UWAGA: LLM parsuje naturalne daty, tutaj testujemy tylko konwersję formatu\n`);
    ...
    test("'jutro' → null (LLM powinien to sparsować)", () => {
        ...
        if (result !== null) {
            throw new Error(`Expected null for 'jutro', got ${result.toDateString()}`);
        }
    });
    test("'wtorek przyszły tydzień' → null (LLM powinien to sparsować)", () => {
        const result = parseNaturalDate("wtorek przyszły tydzień");
        if (result !== null) {
            throw new Error(`Expected null for natural date, got ${result.toDateString()}`);
        }
    });
    // Podsumowanie
    console.log("\n" + "=".repeat(50));
...
export {};
//# sourceMappingURL=voice-action-date-parsing.test.js.map