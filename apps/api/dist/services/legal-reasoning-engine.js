...
export class LegalReasoningEngine {
    ...
    async gatherContext(request) {
        const documents = [];
        if (request.context?.documentIds &&
  ...
  "citations": [
    ...
    parseAnalysisResponse(responseText, context) {
        try {
            ...
            const legalBasis = (parsed.legalBasis || []).map((basis) => {
                const doc = context.documents.find((d) => d.title.includes(basis.documentTitle) ||
            ...
            const citations = (parsed.citations || []).map((citation) => {
                const doc = context.documents.find((d) => d.content.includes(citation.quote));
                return {
                    documentId: doc?.id || "",
...
//# sourceMappingURL=legal-reasoning-engine.js.map