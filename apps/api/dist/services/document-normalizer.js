...
export class DocumentNormalizer {
    ...
    async initialize() {
        ...
        console.log(`[DocumentNormalizer] Initialized with model: ${this.modelName}`);
    }
    /**
    ...
    async normalize(rawDocument) {
        ...
        console.log(`[DocumentNormalizer] Normalizing: "${rawDocument.title.substring(0, 60)}..."`);
        ...
        console.log(`[DocumentNormalizer] Result: type=${validated.documentType}, session=${validated.sessionInfo?.sessionNumber || "N/A"}, confidence=${validated.confidence.overall}%`);
        return validated;
    }
    /**
     * LLM wyodrębnia strukturalne metadane z chaotycznych danych
     */
    async extractMetadataWithLLM(rawDocument) {
...
ODPOWIEDŹ (TYLKO JSON, bez dodatkowego tekstu):`;
     ...
     */
    fallbackExtraction(rawDocument) {
        const title = rawDocument.title.toLowerCase();
        // Prosta detekcja typu
        let documentType = "other";
        if (title.includes("uchwał"))
            documentType = "resolution";
        else if (title.includes("protokół"))
            documentType = "protocol";
        else if (title.includes("transkrypcj"))
     ...
     */
    validateAndEnrich(metadata, rawDocument) {
...
//# sourceMappingURL=document-normalizer.js.map