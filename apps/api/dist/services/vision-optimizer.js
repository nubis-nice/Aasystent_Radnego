...
export class VisionOptimizer {
    ...
    constructor(modelName) {
        ...
        console.log(`[VisionOptimizer] Initialized for model: ${modelName || "default"}, maxDim=${this.config.maxDimension}px`);
    }
    /**
    ...
    async optimizeImage(imageBuffer) {
        ...
        try {
            ...
            // Oblicz nowe wymiary
            const { width, height } = this.calculateDimensions(originalWidth, originalHeight, this.config.maxDimension);
            ...
            console.log(`[VisionOptimizer] Image optimized: ${originalWidth}x${originalHeight} → ${width}x${height}, ` +
                `${this.formatBytes(originalSize)} → ${this.formatBytes(optimizedSize)} (${compressionRatio.toFixed(1)}x)`);
            return {
                buffer: outputBuffer,
     ...
     */
    async optimizeImages(imageBuffers) {
     ...
     */
    buildPrompt(type, additionalContext) {
...
Brak danych = null. NIE zgaduj. TYLKO JSON, bez markdown.`,
    ...
    parseStructuredResponse(response) {
        try {
            ...
            console.error("[VisionOptimizer] Failed to parse structured response:", error);
            return null;
        }
    }
    // ==========================================================================
...
export function getVisionOptimizer(modelName) {
...
// CONVENIENCE EXPORTS
// ============================================================================
export async function optimizeImageForVision(imageBuffer, modelName) {
    const optimizer = getVisionOptimizer(modelName);
    return optimizer.optimizeImage(imageBuffer);
}
export function compressVisionPrompt(prompt, maxTokens) {
 ...
 */
export function getTextToStructuredPrompt() {
    const optimizer = getVisionOptimizer();
    return optimizer.getTextToStructuredPrompt();
}
export function parseDocumentStructure(response) {
    const optimizer = getVisionOptimizer();
    return optimizer.parseStructuredResponse(response);
}
//# sourceMappingURL=vision-optimizer.js.map