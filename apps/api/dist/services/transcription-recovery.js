...
export class TranscriptionRecoveryService {
    ...
    static async recoverStuckJobs() {
        ...
        console.log(`[TranscriptionRecovery] Found ${stuckJobs.length} potentially stuck jobs`);
        let recovered = 0;
        let failed = 0;
        for (const job of stuckJobs) {
            try {
                ...
                if (!queueStatus) {
                    ...
                    console.log(`[TranscriptionRecovery] Job ${job.id} not in queue - marking as failed`);
                    await supabase
                        .from("transcription_jobs")
                        .update({
                        status: "failed",
                    ...
                    console.log(`[TranscriptionRecovery] Job ${job.id} failed in queue - updating DB`);
                    await supabase
                        .from("transcription_jobs")
                        .update({
                        status: "failed",
                    ...
                    console.log(`[TranscriptionRecovery] Job ${job.id} still in queue - OK`);
                    recovered++;
                }
            }
            catch (err) {
                console.error(`[TranscriptionRecovery] Error processing job ${job.id}:`, err);
            }
        }
        console.log(`[TranscriptionRecovery] Recovery complete: ${recovered} recovered, ${failed} marked as failed`);
        return { recovered, failed };
    }
    /**
     * Oznacz zadania które przekroczyły timeout
     */
    static async markTimeoutJobs() {
        ...
        console.log(`[TranscriptionRecovery] Found ${timeoutJobs.length} timeout jobs`);
    ...
    static async runRecoveryCycle() {
        ...
        console.log(`[TranscriptionRecovery] Cycle complete: ${stuck.recovered} recovered, ${stuck.failed + timeout} failed, ${cleaned} cleaned`);
    }
}
/**
 * Automatyczny recovery przy starcie aplikacji
 */
export async function initializeTranscriptionRecovery() {
...
//# sourceMappingURL=transcription-recovery.js.map