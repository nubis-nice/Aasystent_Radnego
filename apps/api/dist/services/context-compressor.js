...
export function estimateTokens(text) {
    ...
    return Math.ceil(baseTokens + newlines + specialChars + headers + polishChars);
}
/**
...
export function compressText(text, options = {}) {
    ...
    if (estimateTokens(compressed) > opts.maxTokens) {
        compressed = truncateParagraphs(compressed, opts.maxTokens, opts.preserveStructure);
    }
 ...
 */
function truncateParagraphs(text, maxTokens, preserveStructure) {
 ...
 */
export function compressRAGContext(documents, municipalData, maxTokens = 8000) {
    // Oblicz oryginalne tokeny
    const originalTokens = documents.reduce((sum, d) => sum + estimateTokens(d.content), 0) +
    ...
    const sortedMunicipal = [...municipalData].sort((a, b) => b.relevanceScore - a.relevanceScore);
    ...
    const compressedMunicipal = compressDocumentList(sortedMunicipal, municipalBudget);
    const totalTokens = compressedDocs.reduce((sum, d) => sum + estimateTokens(d.content), 0) +
 ...
 */
function compressDocumentList(docs, maxTokens) {
    const result = [];
    let currentTokens = 0;
    ...
    for (const doc of docs) {
        ...
        const availableTokens = Math.min(docTokens, maxTokens - currentTokens - titleTokens);
 ...
 */
export function compressConversationHistory(messages, maxTokens = 2000, keepLastN = 4) {
 ...
 */
function createConversationSummary(messages, maxTokens) {
    ...
    for (const msg of messages) {
        if (msg.role === "user") {
            ...
            const firstSentence = (msg.content.split(/[.!?]/)[0] || "").substring(0, 150);
 ...
 */
export function optimizeContext(systemPrompt, ragDocuments, ragMunicipalData, conversationHistory, userMessage, modelName = "default", maxCompletionTokens = 2000) {
    // Określ limit kontekstu dla modelu
    const modelLimit = MODEL_CONTEXT_LIMITS[modelName] ?? MODEL_CONTEXT_LIMITS["default"] ?? 8000;
    const availableTokens = modelLimit - maxCompletionTokens;
    // Oblicz oryginalne tokeny
    const originalSystemTokens = estimateTokens(systemPrompt);
    const originalRagTokens = ragDocuments.reduce((sum, d) => sum + estimateTokens(d.content) + estimateTokens(d.title), 0) +
        ragMunicipalData.reduce((sum, d) => sum + estimateTokens(d.content) + estimateTokens(d.title), 0);
    const originalHistoryTokens = estimateMessagesTokens(conversationHistory);
    const userMessageTokens = estimateTokens(userMessage);
    const originalTotal = originalSystemTokens +
        originalRagTokens +
    ...
    const compressedRag = compressRAGContext(ragDocuments, ragMunicipalData, budget.ragContext);
    // Kompresuj historię
    const compressedHistory = compressConversationHistory(conversationHistory, budget.history);
    // Zbuduj wiadomość RAG
    let ragContextMessage;
    if (compressedRag.documents.length > 0 ||
        compressedRag.municipalData.length > 0) {
        ragContextMessage = buildRAGMessage(compressedRag);
    }
    // Oblicz finalne tokeny
    const compressedTotal = originalSystemTokens +
 ...
 */
function buildRAGMessage(context) {
