...
export class IntelligentRAGSearch {
    ...
    async initialize() {
        ...
        try {
            ...
            console.log(`[IntelligentRAG] Initialized with model: ${this.embeddingModel}`);
        }
        catch (error) {
    ...
    async search(query) {
        ...
        try {
            ...
            const { normalized, entities, variants } = await this.normalizeQuery(query.query);
            result.query.normalized = normalized;
            result.query.extractedEntities = entities;
            console.log(`[IntelligentRAG] Query: "${query.query}" → Normalized: "${normalized}"`);
        ...
        console.log(`[IntelligentRAG] Completed in ${result.searchStats.processingTimeMs}ms, found ${result.totalFound} docs`);
        return result;
    }
    // ============================================================================
    ...
    async normalizeQuery(query) {
            ...
            for (const match of matches) {
                ...
                if (/^[IVXLC]+$/i.test(value)) {
                    ...
                    if (arabicNum > 0 && arabicNum <= 200) {
                        ...
                        const arabicVariant = normalized.replace(new RegExp(value, "gi"), arabicNum.toString());
                    ...
                    if (arabicNum > 0 && arabicNum <= 200) {
                        ...
                        const romanVariant = normalized.replace(new RegExp(value, "g"), romanNum);
        ...
        // Wykryj uchwały
        const resolutionPattern = /uchwa[łl][yaęi]?\s+(?:nr\.?\s*)?([IVXLC\d]+(?:\/\d+)?)/gi;
        ...
        for (const entity of entities) {
            if (entity.type === "session" && /^[IVXLC]+$/i.test(entity.value)) {
                ...
                normalized = normalized.replace(new RegExp(`\\b${entity.value}\\b`, "gi"), entity.normalized);
            }
        }
    ...
    async semanticSearch(query, options) {
        if (!this.embeddingsClient) {
            console.log("[IntelligentRAG] No embeddings client, falling back to keyword search");
            return [];
        }
        try {
            ...
            return (documents || []).map((doc) => this.transformDocument(doc, "semantic"));
        }
        catch (error) {
            console.error("[IntelligentRAG] Semantic search error:", error);
            return [];
    ...
    async findSimilarDocuments(documentId) {
        try {
            ...
            const { data: similar } = await supabase.rpc("search_processed_documents", {
    ...
    // HELPERS
    // ============================================================================
    transformDocument(doc, matchType) {
        return {
            id: doc.id,
            title: doc.title || "Bez tytułu",
            content: doc.content || "",
    ...
    createExcerpt(content, highlightQuery) {
        if (!content)
            return "";
    ...
    async getDebugInfo(queryVariants) {
    ...
    // TEST METHOD
    // ============================================================================
    async runDiagnostics(testQuery) {
        await this.initialize();
        // Statystyki bazy
        const { count: totalDocs } = await supabase
            .from("processed_documents")
        ...
        if ((docsWithEmbedding || 0) < (totalDocs || 0)) {
            recommendations.push(`${(totalDocs || 0) - (docsWithEmbedding || 0)} dokumentów nie ma embeddingów - uruchom regenerację`);
        }
        if (searchResults.totalFound === 0) {
            recommendations.push("Brak wyników - sprawdź czy zapytanie zawiera właściwe słowa kluczowe");
            recommendations.push("Spróbuj użyć numerów arabskich zamiast rzymskich (np. '23' zamiast 'XXIII')");
        }
        if (searchResults.searchStats.semanticMatches === 0 &&
            searchResults.searchStats.keywordMatches > 0) {
            recommendations.push("Wyszukiwanie semantyczne nie zwróciło wyników - sprawdź model embeddingów");
        }
        return {
            query: testQuery,
            dbStats: {
...
//# sourceMappingURL=intelligent-rag-search.js.map