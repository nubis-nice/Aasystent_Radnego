...
export class TTSTextProcessor {
    ...
    formatNumbers(text) {
        ...
        // Kwoty pieniężne
        result = result.replace(/(\d+(?:[.,]\d+)?)\s*(PLN|zł|złotych)/gi, "$1 złotych");
        result = result.replace(/(\d+(?:[.,]\d+)?)\s*(EUR|€|euro)/gi, "$1 euro");
        result = result.replace(/(\d+(?:[.,]\d+)?)\s*(USD|\$|dolarów)/gi, "$1 dolarów");
        return result;
    }
    /**
    ...
    truncateIntelligently(text, maxLength) {
        ...
        if (lastSpace > maxLength * 0.8) {
            return (truncated.substring(0, lastSpace) + "... Odpowiedź została skrócona.");
        }
        return truncated + "...";
    }
    /**
