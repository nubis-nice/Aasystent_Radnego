...
export class SessionDiscoveryService {
     ...
     */
    async discoverSession(intent) {
        const { sessionNumber, requestType } = intent;
        console.log(`[SessionDiscovery] Starting discovery for session ${sessionNumber}, type=${requestType}`);
        // KROK 1: Szukaj w RAG
        const ragDocuments = await this.documentQueryService.findSessionDocuments(sessionNumber);
        // Kategoryzuj znalezione dokumenty
        // Rozszerzone wykrywanie typów - uwzględnia różne nazewnictwo w bazie
        const hasTranscription = ragDocuments.some((d) => d.documentType === "transkrypcja" ||
            d.documentType === "transcription" ||
            d.title.toLowerCase().includes("transkrypcja") ||
            d.title.toLowerCase().includes("stenogram"));
        const hasProtocol = ragDocuments.some((d) => d.documentType === "protokol" ||
        ...
        const hasSessionMaterials = ragDocuments.some((d) => d.documentType === "resolution" || // Porządek obrad sesji
            d.documentType === "session" ||
            d.title.toLowerCase().includes("sesja nr") ||
            d.title.toLowerCase().includes("porządek obrad"));
        const hasVideo = ragDocuments.some((d) => d.sourceUrl?.includes("youtube.com") ||
            d.sourceUrl?.includes("youtu.be"));
        console.log(`[SessionDiscovery] RAG results: ${ragDocuments.length} docs, transcription=${hasTranscription}, protocol=${hasProtocol}, sessionMaterials=${hasSessionMaterials}, video=${hasVideo}`);
        // Jeśli mamy wystarczające dane - zwróć
        // Dla streszczenia akceptujemy: transkrypcję LUB protokół LUB materiały sesji
        if (this.hasRequiredData(requestType, hasTranscription, hasProtocol || hasSessionMaterials, // Rozszerzone - materiały sesji liczą się jako protokół
        ragDocuments.length > 0)) {
            return this.buildSuccessResult(intent, ragDocuments, hasTranscription, hasProtocol, hasVideo);
        }
        ...
        if (youtubeResults.length > 0) {
            console.log(`[SessionDiscovery] Found ${youtubeResults.length} YouTube videos`);
            // Jeśli użytkownik chce transkrypcję/streszczenie - zaproponuj
            if (requestType === "streszczenie" ||
                ...
                const existingJob = await this.checkExistingTranscriptionJob(firstResult.videoId);
                if (existingJob) {
                    return this.buildTranscriptionInProgressResult(intent, ragDocuments, existingJob, hasProtocol);
                }
                // Zaproponuj rozpoczęcie transkrypcji
                return this.buildYouTubeFoundResult(intent, ragDocuments, youtubeResults, hasProtocol);
            }
        }
    ...
    // PRIVATE METHODS
    // ============================================================================
    hasRequiredData(requestType, hasTranscription, hasProtocol, hasAnyDocs) {
        switch (requestType) {
            case "streszczenie":
     ...
     */
    async searchYouTubeDataSources(sessionNumber) {
    ...
    async checkExistingTranscriptionJob(videoId) {
    ...
    // RESULT BUILDERS
    // ============================================================================
    buildSuccessResult(intent, documents, hasTranscription, hasProtocol, hasVideo) {
        ...
        if (hasTranscription) {
            const transcription = documents.find((d) => d.documentType === "transkrypcja");
    ...
    buildYouTubeFoundResult(intent, ragDocuments, youtubeResults, hasProtocol) {
        const romanNumber = this.arabicToRoman(intent.sessionNumber);
    ...
    buildTranscriptionInProgressResult(intent, ragDocuments, job, hasProtocol) {
        const romanNumber = this.arabicToRoman(intent.sessionNumber);
    ...
    buildNotFoundResult(intent, ragDocuments) {
        const romanNumber = this.arabicToRoman(intent.sessionNumber);
        ...
        if (ragDocuments.length > 0) {
            ...
            return {
                ...
                documents: ragDocuments,
                hasTranscription: false,
                hasProtocol: ragDocuments.some((d) => d.title.toLowerCase().includes("protokół")),
...
//# sourceMappingURL=session-discovery-service.js.map