...
export class SemanticWebSearchService {
        ...
        try {
            ...
            for (const result of rawResults) {
                ...
                const crossRefs = query.requireCrossReference
                    ...
                    : [];
                const weightedScore = this.calculateWeightedScore(credibility, crossRefs);
            ...
            const overallConfidence = this.calculateOverallConfidence(filteredResults);
            // 7. Dodaj globalne ostrzeżenia
            if (filteredResults.length < 3) {
                warnings.push("Mało wiarygodnych źródeł - zalecana dodatkowa weryfikacja");
            }
            const contradictions = this.detectContradictions(filteredResults);
            if (contradictions.length > 0) {
                warnings.push(`Wykryto sprzeczne informacje w ${contradictions.length} źródłach`);
            }
            return {
                success: true,
                query: query.query,
    ...
    async performWebSearch(query) {
        ...
        if (braveApiKey) {
            try {
                const braveResults = await this.searchWithBrave(query.query, braveApiKey, query.maxResults || 10);
        ...
        if (results.length === 0) {
            try {
                const ddgResults = await this.searchWithDuckDuckGo(query.query, query.maxResults || 10);
        ...
        if (query.excludeDomains && query.excludeDomains.length > 0) {
            filteredResults = filteredResults.filter((r) => !query.excludeDomains.some((d) => r.domain.includes(d)));
        }
        if (query.preferredDomains && query.preferredDomains.length > 0) {
            const preferred = filteredResults.filter((r) => query.preferredDomains.some((d) => r.domain.includes(d)));
            const others = filteredResults.filter((r) => !query.preferredDomains.some((d) => r.domain.includes(d)));
     ...
     */
    async searchWithBrave(query, apiKey, count) {
     ...
     */
    async assessCredibility(result, originalQuery) {
        const flags = [];
        ...
        const contentAnalysis = await this.analyzeContentQuality(result, originalQuery);
        // 4. Freshness score
        const freshness = this.calculateFreshness(result.publishedDate);
        // 5. Combine scores
        const overall = Math.round(domainTrust * 0.3 +
     ...
     */
    async analyzeContentQuality(result, originalQuery) {
        if (!this.llmClient) {
...
Tytuł: "${result.title}"`,
     ...
     */
    async crossReference(result, allResults) {
...
{ "claims": ["twierdzenie1", "twierdzenie2"] }`,
            ...
            const { claims } = JSON.parse(claimsResponse.choices[0]?.message?.content || '{"claims":[]}');
            const crossRefs = [];
...
Tekst: "${otherContent.substring(0, 1000)}"`,
                    ...
                    const verdict = verifyResponse.choices[0]?.message?.content
                        ?.toLowerCase()
                        .trim();
                    if (verdict?.includes("supports"))
                        supporting++;
                    else if (verdict?.includes("contradicts"))
                        contradicting++;
                }
                crossRefs.push({
                    claim,
                    ...
                    confidence: supporting > contradicting
     ...
     */
    calculateWeightedScore(credibility, crossRefs) {
        ...
        if (crossRefs.length > 0) {
            const avgConfidence = crossRefs.reduce((sum, cr) => sum + cr.confidence, 0) /
                crossRefs.length;
     ...
     */
    calculateFreshness(publishedDate) {
        if (!publishedDate)
        ...
        for (const result of results) {
            for (const crossRef of result.crossReferences) {
                if (crossRef.contradictingSources > crossRef.supportingSources) {
                    contradictions.push(`"${crossRef.claim}" - ${crossRef.contradictingSources} źródeł zaprzecza`);
                }
            }
        }
...
// EXPORTED FUNCTION
// ============================================================================
export async function semanticWebSearch(userId, query) {
    const service = new SemanticWebSearchService(userId);
    return service.search(query);
}
//# sourceMappingURL=semantic-web-search.js.map