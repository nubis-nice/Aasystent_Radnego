...
export class AudioAnalyzer {
    ...
    async analyze(inputPath) {
        ...
        // Wykryj problemy
        const issues = this.detectIssues(volumeInfo, loudnessInfo, dynamicRange, basicInfo);
        // Wygeneruj rekomendacje
        const recommendations = this.generateRecommendations(volumeInfo, loudnessInfo, dynamicRange, issues);
        const analysis = {
            ...basicInfo,
            ...volumeInfo,
            ...loudnessInfo,
    ...
    async analyzeVolume(inputPath) {
        return new Promise((resolve, reject) => {
            ...
            ffmpeg.on("error", (error) => {
                ...
                console.warn(`[AudioAnalyzer] volumedetect error: ${error.message}, using defaults`);
    ...
    async analyzeLoudness(inputPath) {
        return new Promise((resolve, reject) => {
            ...
            ffmpeg.on("close", (code) => {
                ...
                const jsonMatch = stderr.match(/\{[\s\S]*"input_i"[\s\S]*"input_lra"[\s\S]*\}/);
                if (jsonMatch) {
                    try {
            ...
            ffmpeg.on("error", (error) => {
                console.warn(`[AudioAnalyzer] loudnorm error: ${error.message}, using defaults`);
                resolve({
                    integratedLoudness: -23,
                    loudnessRange: 7,
                    truePeak: -1,
     ...
     */
    detectIssues(volumeInfo, loudnessInfo, dynamicRange, basicInfo) {
        const issues = [];
        // Zbyt cicho
        if (volumeInfo.meanVolume < -35) {
            issues.push({
                ...
                description: `Średnia głośność ${volumeInfo.meanVolume.toFixed(1)}dB jest zbyt niska`,
            });
        }
        ...
        if (volumeInfo.maxVolume > -0.5) {
            issues.push({
                ...
                description: `Szczytowa głośność ${volumeInfo.maxVolume.toFixed(1)}dB - możliwe przesterowanie`,
            });
        }
        ...
        if (dynamicRange > 25) {
            issues.push({
                ...
                description: `Zakres dynamiki ${dynamicRange.toFixed(1)}dB - duże różnice głośności`,
            });
        }
        ...
        if (loudnessInfo.loudnessRange > 15) {
            issues.push({
                ...
                description: `Zakres głośności ${loudnessInfo.loudnessRange.toFixed(1)} LU - wymaga kompresji`,
            });
        }
        // Niska jakość źródła
     ...
     */
    generateRecommendations(volumeInfo, loudnessInfo, dynamicRange, issues) {
        ...
        if (hasIssue("too_quiet")) {
            ...
            console.log(`[AudioAnalyzer] Recommending gain boost: +${boost.toFixed(1)}dB`);
        }
        ...
        if (hasIssue("high_dynamic_range")) {
            ...
            console.log(`[AudioAnalyzer] Recommending stronger compression: ratio=${recommendations.compressorRatio}`);
        }
        // Szum - silniejsza redukcja szumów
        if (hasIssue("noise")) {
            recommendations.noiseFloor = -20;
            recommendations.highpassFreq = 120; // Wyższy highpass
            console.log(`[AudioAnalyzer] Recommending stronger noise reduction: floor=${recommendations.noiseFloor}dB`);
        }
